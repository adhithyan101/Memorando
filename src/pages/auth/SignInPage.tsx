...
export const SignInPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Auth context sets readable error message
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Auth context sets readable error
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="min-h-screen bg-bg-light dark:bg-bg-dark flex flex-col items-center justify-center p-6 transition-colors duration-200">
      <div className="max-w-md w-full bg-card-light dark:bg-card-dark rounded-3xl p-8 shadow-warm border border-sand dark:border-sand-dark">
        ...
        <div className="flex items-center space-x-3 mb-6">
          ...
          <div>
            <h1 className="font-serif text-2xl font-bold text-charcoal dark:text-charcoal-dark">Welcome Back</h1>
            <p className="text-xs text-slate dark:text-slate-dark">Sign in to revisit your memories</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label className="block text-xs font-semibold uppercase tracking-wider text-slate dark:text-slate-dark">
                Password
              </label>
              <Link to="/forgot-password" className="text-xs text-terracotta hover:underline font-medium">
                Forgot password?
              </Link>
            </div>
            <div className="relative">
        ...
        <p className="mt-8 text-center text-xs text-slate dark:text-slate-dark">
          Don't have an album yet?{' '}
          <Link to="/create-account" className="text-terracotta hover:underline font-semibold">
            Create Account
          </Link>
        </p>
      </div>
